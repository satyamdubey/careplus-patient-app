      ...
      body: Column(
        children: [
          GetBuilder<AppointmentController>(builder: (_) {
            _myAppointments.assignAll(_appointmentController.allAppointmentList);
        ...
        children: [
          ...
          TableCalendar(
            ...
            onDaySelected: (selectedDay, focusedDay) {
              _myAppointmentController.selectedDay=selectedDay;
            ...
            calendarBuilders: CalendarBuilders(markerBuilder: (context, date, _) {
              return _myAppointments.any((element){
                return (DateFormat('yyyy-MM-dd').parse(element.bookingDate).day == date.day &&
                    DateFormat('yyyy-MM-dd').parse(element.bookingDate).month == date.month);
              }) ? Center(
                child: Container(
                  width: 26,
                  height: 26,
                  ...
                  children: [
                    ...
                    Text.rich(
                      TextSpan(text:'Booking date: ', style: nunitoRegular.copyWith(fontSize: FONT_SIZE_SMALL),
                        children: [
                          TextSpan(
                            text: '${_myAppointmentController.selectedDayAppointments[index].bookingDate}',
                            style: nunitoBold.copyWith(fontSize: FONT_SIZE_SMALL),
                          )
                        ],
                      ),
                    ...
                    Text.rich(
                      TextSpan(text:'Booking status: ', style: nunitoRegular.copyWith(fontSize: FONT_SIZE_SMALL),
                        children: [
                          TextSpan(
                            text: '${_myAppointmentController.selectedDayAppointments[index].status}',
                            style: nunitoBold.copyWith(fontSize: FONT_SIZE_SMALL),
                          )
                        ],
                      ),
                    ...
                    Row(
                      ...
                      children: [
                        ...
                        GestureDetector(
                          ...
                          child: Text('More Details', style: nunitoBold.copyWith(fontSize: FONT_SIZE_SMALL)))
                      ],
                    ),
                  ],
                ),
...
/*
bool isAppointmentUpcoming = false;
bool isAppointmentCompleted = false;
bool isAppointmentCanceled = false;
isAppointmentCompleted = (element.status == "booked" && element.completed == true);
isAppointmentUpcoming = (element.status == "booked" && element.completed == false);
isAppointmentCanceled = (element.status == "canceled" && element.completed == false);
 */
