...
class _SelectPaymentModeScreenState extends State<SelectPaymentModeScreen> {
      ...
      child: Container(
        ...
        child: Column(
          ...
          children: [
            ...
            _textRow(
              'TIME',
              '${appointment.meetHours}:${appointment.meetMinutes}',
              nunitoBold.copyWith(
                color: Colors.grey.shade500,
                fontSize: FONT_SIZE_SMALL,
              ),
  ...
  Future<void> _createAppointment(int selectedMember) async {
    FamilyMember familyMember =
        _familyMemberController.familyMembers[selectedMember];
    ...
    if (selectedMember == 0) {
      ...
      var response = await _appointmentController
          .createAppointmentForFamilyMember(familyMember.id);
      EasyLoading.dismiss();
      if (response != null && response is Appointment) {
        await _initiatePayment(response, total);
      }
  ...
  Future<void> _initiatePayment(Appointment appointment, double amount) async {
    ...
    if (tokenResponse is http.Response && tokenResponse.statusCode == 201) {
      if (jsonDecode(tokenResponse.body)['status'] == 'Success') {
        ...
        var txnResponse = await PaymentService.initiateTransaction(orderId, txnToken, amount);
        if (txnResponse != null && txnResponse['STATUS'] == 'TXN_SUCCESS') {
          var txnId = txnResponse['TXNID'];
          EasyLoading.show(status: 'processing please wait');
          await PaymentService.sendTransactionResponse(orderId, txnId);
          EasyLoading.dismiss();
          showDialog(context: context, barrierDismissible: false, builder: (_) => _showOrderConfirmDialog(appointment, txnId));
