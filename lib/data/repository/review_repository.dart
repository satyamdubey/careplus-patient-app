...
class ReviewRepository {
  static Future<dynamic> createClinicReview(ClinicRating clinicRating) async {
    ...
    if (response is http.Response && response.statusCode == 200) {
      ClinicReviewData clinicReviewData =
          clinicReviewDataFromJson(response.body);
  ...
  static Future<dynamic> createDoctorReview(DoctorRating doctorRating) async {
    ...
    if (response is http.Response && response.statusCode == 200) {
      DoctorReviewData doctorReviewData =
          doctorReviewDataFromJson(response.body);
  ...
  static Future<dynamic> getClinicReviews(String clinicId) async {
    var response =
        await ApiClient().getData(ApiConstant.getClinicRatings + clinicId);
  ...
  static Future<dynamic> getDoctorReviews(String doctorId) async {
    var response =
        await ApiClient().getData(ApiConstant.getDoctorRatings + doctorId);
    if (response.statusCode == 200) {
  ...
  static Future<dynamic> reportDoctor(String doctorId) async {
    var response =
        await ApiClient().getData(ApiConstant.reportDoctor + doctorId);
    if (response.statusCode == 200) {
      return response;
    } else {
      return null;
  ...
  static Future<dynamic> reportClinic(String clinicId) async {
    var response =
        await ApiClient().getData(ApiConstant.reportClinic + clinicId);
    if (response.statusCode == 200) {
      return response;
    } else {
      return null;
